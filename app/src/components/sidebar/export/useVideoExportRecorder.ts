...
export function useVideoExportRecorder() {
  ...
  const captureFrame = useCallback(() => {
    ...
    if (videoExportSettings.backgroundMode === 'solid') {
      const bgColor = videoExportSettings.backgroundColor === 'green' 
        ? '#00ff00' 
    ...
    // Draw map only if background mode is 'map'
    if (videoExportSettings.backgroundMode === 'map') {
      context.drawImage(
        mapCanvas,
        cropX * pixelScaleX,
        cropY * pixelScaleY,
    ...
    // Only draw marker if map background is enabled
    if (videoExportSettings.backgroundMode === 'map') {
    ...
    // Draw logo only if enabled
    if (videoExportSettings.showLogo && cachedLogoRef.current) {
